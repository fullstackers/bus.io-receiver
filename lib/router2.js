...
function Router () {
  ...
  this._attachEvents = function () {
    debug('attach handlers to controller', arguments[0]);
    var args = slice.call(arguments);
    var msg = args.shift(); // the controller at this point
    var end = args.pop();
    var next = args.pop();
    ['deliver', 'respond', 'consume'].forEach(function (name) { 
      msg.on(name, self._event(name, end)); 
    });
    next();
  };
...
Router.prototype.route = function () {
  ...
  var end = function () {
    debug('end');
    // unshifts "next", shifts the "action", pops the "end"
    var params = ['next'].concat(args.slice(1, args.length - 1))
    params[1] = params[1].message;
    emit.apply(self, params);
    _end();
  };
...
Router.prototype._event = function (name, end) {
  var self = this;
  return function () {
    var args = slice.call(arguments);
    emit.apply(self, [name].concat(args.slice(0, args.length - 1)));
