...
function Router () {
  ...
  this.onDeliver = function () {
    ...
    emit.apply(self, ['deliver'].concat(slice.call(arguments)));
  };
  ...
  this.onConsume = function () {
    ...
    emit.apply(self, ['consume'].concat(slice.call(arguments)));
  };
  ...
  this.onRespond = function () {
    ...
    emit.apply(self, ['respond'].concat(slice.call(arguments)));
  };
  ...
  this.onNext = function () {
    ...
    emit.apply(self, ['next'].concat(slice.call(arguments)));
  };
...
Router.prototype.route = function (message, done) {/* done() is for testing */
  ...
  if (util.isArray(message) && (!message.length || !(message[0] instanceof Message))) throw new Error('the first item must be a Message');
  if (!util.isArray(message) && !(message instanceof Message)) throw new Error('message must be a Message');
  if (util.isArray(message)) return this.getRoute(message[0].action()).process(message, done);
