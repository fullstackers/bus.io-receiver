var util = require('util')
  ...
  , debug = require('debug')('bus.io-receiver-route')
  , async = require('async')
...
Route.prototype.process = function (message, done) {/* done() is for testing */
  ...
  var fns = [
    ...
    function (cb) {
      ...
      // this needs set if we happen to call deliver in the processing of this
      // message in a previous receiver
      ...
      controller.on('deliver', function (message) {
        ...
        emit.apply(self, ['deliver'].concat(args).concat(params));
      ...
      controller.on('respond', function (message) {
        ...
        var args = slice.call(arguments);
        emit.apply(self, ['respond'].concat(args).concat(params));
      ...
      controller.on('consume', function (message) {
        ...
        var args = slice.call(arguments);
        emit.apply(self, ['consume'].concat(args).concat(params));
      ...
      if (typeof done === 'function') {
        ...
        controller.on('deliver', function () {
          ...
          done.apply(done, slice.call(arguments));
        });
  ...
  async.waterfall(fns, function (err, args) {
    ...
    if (err && err !== true) {
      ...
      emit.apply(self, ['next', message].concat(params));
      if (typeof done === 'function') {
